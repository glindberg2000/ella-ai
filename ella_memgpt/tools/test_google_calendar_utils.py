...
class TestGoogleCalendarUtils(unittest.TestCase):
    ...
    def test_fetch_upcoming_events(self, mock_get_user_email, mock_build):
        ...
        events = calendar_utils.fetch_upcoming_events('test_user_id')
        self.assertEqual(len(events), 2)
        self.assertEqual(events[0]['summary'], 'Test Event')
        self.assertEqual(events[0]['id'], 'test_event_id')
        self.assertEqual(events[1]['summary'], 'Test Recurring Event')
        self.assertEqual(events[1]['id'], 'test_recurring_event_instance_id')
        self.assertEqual(events[1]['recurringEventId'], 'test_recurring_event_id')
