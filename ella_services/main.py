...
async def token_refresh_task():
    while True:
        try:
            expiries = google_service_manager.get_token_expiry()
            now = datetime.utcnow()

            for service, expiry in expiries.items():
                if expiry and (expiry - now) < timedelta(minutes=30):
                    logger.info(f"Token for {service} is expiring soon. Refreshing...")
                    google_service_manager.refresh_all_tokens()
                    break
            
            await asyncio.sleep(30)  # Check every 15 minutes
        except Exception as e:
