        ...
        try:
            ...
            for user in active_users:
                ...
                if user_data:
                    ...
                    current_time = current_time_utc.astimezone(pytz.timezone(user_timezone))
                    # system_time = time.strftime('%Y-%m-%d %H:%M:%S %Z', time.localtime())
                    # ntp_time = get_ntp_time()

                    # Log each time calculation exactly once
                    #logger.info(f"Current UTC time: {current_time_utc}")
                    logger.info(f"Current User time ({user_timezone}): {current_time}")
                    # logger.info(f"System time: {system_time}")
                    # logger.info(f"UTC time: {datetime.now(pytz.UTC)}")  # Log UTC time separately
                    # if ntp_time:
